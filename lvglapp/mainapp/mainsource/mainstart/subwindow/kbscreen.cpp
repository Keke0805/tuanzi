...
void kbscreen::kb_textarea_event_hander(lv_event_t *event)
{
    ...
    const char *txt = lv_btnmatrix_get_btn_text(opt->kb->myobj,lv_btnmatrix_get_selected_btn(opt->kb->myobj));
        if(txt == NULL)
            return;
        if(target==(opt->kb->myobj))
        {
