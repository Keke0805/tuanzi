...
void Statical::ui_image_decode(void)
{
    ...
    MAIN_MENU_BACKGROUND_CLASSIC_JPG=(void*)parse_image_from_file((char*)MAIN_MENU_CLASSIC_JPG_PATH);
...
{
    if(MAIN_MENU_BACKGROUND_CLASSIC_JPG !=NULL)
        {
            free_image(MAIN_MENU_BACKGROUND_CLASSIC_JPG);
            MAIN_MENU_BACKGROUND_CLASSIC_JPG =NULL;
        }
...
Statical::Statical(lv_obj_t* parent)
{
    ...
    mscreen->showlogo(MAIN_MENU_CLASSIC_JPG_PATH,SCREEN_HOR_SIZE,SCREEN_VER_SIZE,0);
    initial();
}
...
void Statical::initial(void)
{
    ...
    lvgl_scroll_anim();
    /*
    background = new LvcppImage(m_parent);
    background->set_main_bg_img_src(MAIN_MENU_BACKGROUND_BUTTERFLIES_JPG);
    background->set_size(SCREEN_HOR_SIZE,SCREEN_VER_SIZE);
    background->set_main_bg_opa(LV_OPA_0);
    */
