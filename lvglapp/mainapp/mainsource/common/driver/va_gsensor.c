...
int va_gsensor_init(va_gsensor_callback_fun fun)
{
	...
	gd = (va_GsensorDataType*)malloc(sizeof(va_GsensorDataType));
	if(NULL == gd)
	{
		printf("malloc fail\n");
		return -1;
	}
	memset((void *)gd, 0, sizeof(va_GsensorDataType));
