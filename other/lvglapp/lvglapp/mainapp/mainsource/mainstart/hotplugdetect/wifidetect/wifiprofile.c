...
int createWifiDefaultConfig()
{
	...
	obj = cJSON_CreateObjectToObject(root, "wifi");
	cJSON_AddBoolToObject(obj, "isSupport", g_bWifiSupport);
	cJSON_AddBoolToObject(obj, "isEnable", g_bWifiEnable);
	cJSON_AddStringToObject(obj, "ssid", "");
	cJSON_CreateArrayToObject(obj, "ssidlist");
	connections = cJSON_CreateObjectToObject(obj, "connections");
...
int addSsidInfoToConnList(char *ssid, Wifi_ConnectInfo_t *pConnInfo)
{
	...
	conn = cJSON_CreateObjectToObject(g_pConnect, ssid);
	memset(index, 0, sizeof(index));
	sprintf(index, "%d", pConnInfo->index);
	cJSON_AddStringToObject(conn, "index", index);
	cJSON_AddStringToObject(conn, "passwd", pConnInfo->passwd);
