...
static void *CheckUsbHotPlugProc(void *pdata)
{
    ...
    char mountCmd[CMD_BUFFER_SIZE] = {0};
    char rmCmd[CMD_BUFFER_SIZE] = {0};
    int nRet = 0;
    UsbParam_t tmpUsbParam;
    ...
    while(g_bCheckUsbThreadRun)
    {
        ...
        if (strstr(buf, "sd") && (strlen(strstr(buf, "sd")) > DEV_NAME_LEN_MIN))
        {
			...
			memset(&tmpUsbParam, 0, sizeof(UsbParam_t));
			...
			if (strstr(buf, "add"))
			{
				...
				{
					tmpUsbParam.action = 1;
					sprintf(tmpUsbParam.udisk_path, "/vendor/udisk_%s", pstmsg);
					memset(mountCmd, 0, CMD_BUFFER_SIZE);
					sprintf(mountCmd, "mkdir -m 777 %s",tmpUsbParam.udisk_path);
					system(mountCmd);
					system("sync");
					memset(mountCmd, 0, CMD_BUFFER_SIZE);
					sprintf(mountCmd, "mount /dev/%s %s",pstmsg,tmpUsbParam.udisk_path);
					system(mountCmd);
					system("sync");
					if (access(tmpUsbParam.udisk_path, F_OK) == 0)
					{
						printf("udisk path: %s\n",tmpUsbParam.udisk_path);
						memset(&stUsbParam, 0, sizeof(UsbParam_t));
						memcpy(&stUsbParam, &tmpUsbParam, sizeof(UsbParam_t));
						...
						list_for_each(pListPos, &g_usbCallbackListHead)
						{
							...
							pstUsbCallbackData->pfnCallback(&tmpUsbParam);
						}
			...
			if (strstr(buf, "remove"))
			{
				...
				if (access(tmpUsbParam.udisk_path, F_OK) == 0)
				{
					...
					sprintf(umountCmd, "umount %s", tmpUsbParam.udisk_path);
					...
					sprintf(rmCmd, "rm -rf %s", tmpUsbParam.udisk_path);
					system(rmCmd);
					memset(rmCmd, 0, CMD_BUFFER_SIZE);
					sprintf(rmCmd, "rm -rf /vendor/*");
					system(rmCmd);
				}
			}
        }
