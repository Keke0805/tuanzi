...
void lv_draw_g2d_blend(lv_draw_ctx_t * draw_ctx, const lv_draw_g2d_blend_dsc_t * dsc)
{
    ...
    ((lv_draw_g2d_ctx_t *)draw_ctx)->blend(draw_ctx, dsc);
}

LV_ATTRIBUTE_FAST_MEM void lv_draw_g2d_blend_basic(lv_draw_ctx_t * draw_ctx, const lv_draw_g2d_blend_dsc_t * dsc)
{
    const lv_opa_t * mask;
    ...
    draw_area_size = lv_area_get_size(dsc->blend_area);
    ...
    if(disp->driver->set_px_cb) {
        ...
        if(dsc->blend_mode == LV_BLEND_MODE_NORMAL) {
            ...
            if (draw_area_size >= sunxifb_g2d_get_limit(SUNXI_G2D_LIMIT_FILL))
            {
                if(!sunxifb_g2d_fill(dest_buf, &blend_area,dsc->blend_area, dsc->color, dsc->opa))
                {
                    return;
                }
            }
        ...
        if(dsc->blend_mode == LV_BLEND_MODE_NORMAL) {
            ...
            if(draw_area_size >= sunxifb_g2d_get_limit(SUNXI_G2D_LIMIT_BLEND))
            {
                if(!sunxifb_g2d_blend(dest_buf, &blend_area, dsc->blend_area, dsc->src_buf, dsc->blend_area,dsc->opa,false))
                {
                    return;
                }
            }
            #endif /* LV_USE_SUNXIFB_G2D_BLEND */
            map_normal(dest_buf, &blend_area, dest_stride, dsc->src_buf, src_stride, dsc->opa, mask, mask_stride);
        }
#if LV_DRAW_COMPLEX
        else {
            #if LV_USE_SUNXIFB_G2D_BLEND
            ...
            #endif /* LV_USE_SUNXIFB_G2D_BLEND */
            map_blended(dest_buf, &blend_area, dest_stride, dsc->src_buf, src_stride, dsc->opa, mask, mask_stride, dsc->blend_mode);
        }
#endif
    }
}
