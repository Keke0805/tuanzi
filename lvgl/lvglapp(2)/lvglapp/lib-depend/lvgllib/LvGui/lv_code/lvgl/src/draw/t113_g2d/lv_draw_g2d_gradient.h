/**
 * @file lv_draw_g2d_gradient.h
 *
 */

#ifndef LV_DRAW_G2D_GRADIENT_H
#define LV_DRAW_G2D_GRADIENT_H
 ...
 */
LV_ATTRIBUTE_FAST_MEM lv_grad_color_t lv_g2d_gradient_calculate(const lv_grad_dsc_t * dsc, lv_coord_t range,
                                                            lv_coord_t frac);
 ...
 */
void lv_g2d_gradient_set_cache_size(size_t max_bytes);

/** Free the gradient cache */
void lv_g2d_gradient_free_cache(void);

/** Get a gradient cache from the given parameters */
lv_grad_t * lv_g2d_gradient_get(const lv_grad_dsc_t * gradient, lv_coord_t w, lv_coord_t h);

/**
 * Clean up the gradient item after it was get with `lv_grad_get_from_cache`.
 * @param grad      pointer to a gradient
 */
void lv_g2d_gradient_cleanup(lv_grad_t * grad);

#ifdef __cplusplus
} /*extern "C"*/
#endif

#endif /*LV_DRAW_G2D_GRADIENT_H*/
