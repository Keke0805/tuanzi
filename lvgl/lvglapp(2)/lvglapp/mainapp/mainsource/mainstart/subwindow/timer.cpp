...
void Timer::TimerCB(lv_timer_t * tmr)
{
    ...
    char time_second[1024];
    user_time = gettime.current_datetime();
    sprintf(time_current,"%d-%02d-%02d \n %02d:%02d:%02d",user_time.year,user_time.month,user_time.day,user_time.hour,user_time.minute,user_time.second);
    //sprintf(time_second,"%02d:%02d:%02d",user_time.hour,user_time.minute,user_time.second);
    LV_LOG_USER("%s",time_current);
    time_value->add_txt(time_current);
