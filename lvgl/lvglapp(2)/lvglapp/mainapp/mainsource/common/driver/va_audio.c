...
//amixer cset name="Soft Volume Master"  255
...
p_fftw3_callback fftw3_callback = NULL;

void register_fftw3_hook(p_fftw3_callback func)
{
    fftw3_callback = func;
}
...
{
    float len_mult = 1.0f/(float)(len-1);
    for (int i = 0; i < len; i++) {
        window[i] = 0.5f * (1.0f - cos((float)i * 2.0f * M_PI * len_mult));
		if(len%16==0)
		printf("window[%d]=%.2f",i,window[i]);
    }
}

int va_fftw3_manage(char* buf,unsigned int SampleRate)
{
	char* in_buf = buf;
	float Input[BUF_LEN/4]={0};
	float Output[BUF_LEN/4]={0};
	int re, im ,index;
	...
	dsps_wind_hann_f32(Input,len);

	for(int i=0;i<len;++i)
	{
		in[i][0]=(float)(in_buf[i]*Input[i]);
		in[i][1]=(float)0;
	}
...
int va_fftw3_open_wav_file_test2(const char *partname)
{
	int err;
	wave_header_t wav;
	int headwavcntp;
	...
	while (!feof(fp)) {
        ...
        va_fftw3_manage(buf,wav.uSampleRate);
		//fseek(fp,-(BUF_LEN/4),SEEK_CUR);
	}
