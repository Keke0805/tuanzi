...
static char *parse_lrc_get_time_str(char *p, char *buf)
{
	...
	return (char *)(( unsigned long int)p + ( unsigned long int)right - ( unsigned long int)left + 1);
}
...
#if 1
	com_info("lang =%s",lrc->lang);
	com_info("title =%s",lrc->title);
	com_info("artist =%s",lrc->artist);
	com_info("albumArtist =%s",lrc->albumArtist);
	com_info("lrc_author =%s",lrc->lrc_author);
#endif
	if(next == p){
		return NULL;
	}
...
int get_line_text(char *p, char *end, char *buf, int buf_len)
{
	...
	unsigned long int size = (unsigned long int)end - (unsigned long int)p;

	//com_info("size =%d", size);
...
#define LINE_TIME_NUM 8
	...
	while(1){
		...
		if(next[0] != '[')
			break;
		//com_info("%s", str_time[num]);
		num++;

		if(num >= LINE_TIME_NUM){
			com_err("format error");
			return -1;
		}
	}
...
int parse_lrc_get_time_list(music_lrc_info *lrc)
{
	...
	while(1){
		...
		next = next + line_len;
		//com_info("line_buf = %s %d", line_buf, line_len);
		...
		//com_info("next=%p %p %d %d", next, p+len, time_list[i], count);
