	...
	switch(cmd){
		...
		case PREPARE_CMD:
            ...
            if (tplayer->mMediaInfo->pVideoStreamInfo)
            {
                ...
                if (video_resolution[0] > 1920 && video_resolution[1] > 1080)
                {
                    com_info("more than 1080P\r\n");
                    TPlayerStop(tplayer->mTPlayer);
                    tplayer->mpstatus = STOP_STATUS;
                    sem_post(&tplayer->mPreparedSem);
                    if(tplayer->callback)
...
{
	printf("tplayer seekto %d second\n",nSeekTimeSec);
...
{
	printf("tplayer_rotate:rotateDegree=%d\n",rotateDegree);
...
int tplayer_get_volume(player_t *tplayer)
{
	if(!tplayer){
		return -1;
	}
	return TPlayerGetVolume(tplayer->mTPlayer);
}
#endif
