...
int media_get_file_type(media_file_list_t *media_file_list, int index)
{
	int ret = 0;
...
char *media_get_file_name(media_file_list_t *media_file_list, int index)
{
	int ret = 0;
	rat_entry_t ItemInfo;
	...
	extern char * SLIB_strchrlast(char * pstr, char srch_char);
	return (char *)(SLIB_strchrlast(ItemInfo.Path, '/') + 1);
}

int media_get_index_from_path(media_file_list_t *media_file_list,char *path)
{
	int index,ret = 0;
	char listpath[1024];
	if (media_file_list == NULL || path==NULL) 
	{  
		com_err("param error");
		return -1;
	} 
	for(int i=0;i< media_file_list->total_num;i++)
	{  
		memset(listpath,0,sizeof(listpath));  
		ret = rat_npl_index2file(media_file_list->media_hrat,i, listpath);
		if (ret != EPDK_OK) 
		{   
			printf("rat_npl_index2file fail!\n");
			return -1;
		}  
		if(strcmp(listpath,path)==0)  
		{   
			return i;  
		} 
	} 
...
void _get_file_time(char *path,char *time)
{
	struct stat file_info;
	time_t tTimeTmp = 0;
	struct tm stuTimeTmp;
...
void _get_file_size(char *path,char *size)
{
	struct stat file_info;
    stat(path, &file_info);
	file_size_int_to_string(file_info.st_size, size);
