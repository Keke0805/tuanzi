...
HSCAN ScanFilesBegin(const char *pDir, __u64 FileTypes, int OnceCnt)
{
	...
	pList->Prev		= NULL;
	pList->Next			= NULL;
	pList->Alloc		= FILELIST_LEN - offsetof(ScanFileList_t,Data);
	pList->Size		= 0;
	pList->Top			= 0;
								   ...
								   ScanInfo_t *pScanInfo)
{
	...
	RecLen = RAT_ALIGN(offsetof(ScanFileEntry_t,Name) + strlen(filePath) + 1, sizeof(__u64));
	...
	if (RecLen > (pLastList->Alloc - pLastList->Size)) {
		...
		pNewList->Next		= NULL;
		pNewList->Alloc		= FILELIST_LEN - offsetof(ScanFileList_t,Data);
		pNewList->Size		= 0;
		pNewList->Top		= 0;
...
static FmtIdx_t ValidFmtTbl[] = {
   ...
   // { ".MID",		RAT_MEDIA_TYPE_AUDIO,		MUSIC_SCAN_MID   },
	...
	{ ".swu",	RAT_MEDIA_TYPE_FIRMWARE,	IMAGE_SCAN_IMG	},
