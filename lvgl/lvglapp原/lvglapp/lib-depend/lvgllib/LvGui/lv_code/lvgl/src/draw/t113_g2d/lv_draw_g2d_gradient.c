 ...
 *      INCLUDES
 *********************/
#include "lv_draw_g2d_gradient.h"
...
lv_grad_t * lv_g2d_gradient_get(const lv_grad_dsc_t * g, lv_coord_t w, lv_coord_t h)
{
    ...
    if(!inited) {
        lv_g2d_gradient_set_cache_size(LV_GRAD_CACHE_DEF_SIZE);
        inited = true;
    }
...
#else
    for(lv_coord_t i = 0; i < item->size; i++) {
        item->map[i] = lv_g2d_gradient_calculate(g, item->size, i);
    }
#endif
