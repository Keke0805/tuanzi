...
LV_ATTRIBUTE_FAST_MEM void lv_draw_g2d_img_decoded(struct _lv_draw_ctx_t * draw_ctx, const lv_draw_img_dsc_t * draw_dsc,
                                                  const lv_area_t * coords, const uint8_t * src_buf, lv_img_cf_t cf)
{
    ...
    lv_area_t* disp_area = draw_ctx->buf_area;

    bool mask_any = lv_draw_mask_is_any(&draw_area);

    bool chroma_keyed = lv_img_cf_is_chroma_keyed(cf);
    bool alpha_byte   = lv_img_cf_has_alpha(cf);
    ...
    if(!mask_any && draw_dsc->angle == 0 && draw_dsc->zoom == LV_IMG_ZOOM_NONE && cf == LV_IMG_CF_TRUE_COLOR
    && chroma_keyed == false && alpha_byte == false && draw_dsc->recolor_opa == LV_OPA_TRANSP) {
        blend_dsc.blend_area = coords;
        blend_dsc.src_buf = (const lv_color_t *)src_buf;
#if LV_USE_SUNXIFB_G2D_BLIT
        if (lv_area_get_width(&draw_area)*lv_area_get_height(&draw_area)>= sunxifb_g2d_get_limit(SUNXI_G2D_LIMIT_BLIT))
        {
            ...
            //LV_LOG_USER("sunxifb_g2d_blit\n");
            if (!sunxifb_g2d_blit(dest_buf, disp_area, &draw_area, (lv_color_t *)src_buf, &draw_area,draw_dsc->opa))
            {
                return;
            }
        }
    ...
    /*In the other cases every pixel need to be checked one-by-one*/
        ...
        if(!mask_any && !transform && !chroma_keyed &&  cf == LV_IMG_CF_TRUE_COLOR_ALPHA && draw_dsc->recolor_opa == LV_OPA_TRANSP && alpha_byte) {
            ...
            #if LV_USE_SUNXIFB_G2D_BLEND
            if(draw_area_h*draw_area_w >= sunxifb_g2d_get_limit(SUNXI_G2D_LIMIT_BLEND))
            {
                /*Make it relative to `disp_area` to simplify draw to `disp_buf`*/
                draw_area.x1 -= disp_area->x1;
                draw_area.y1 -= disp_area->y1;
                draw_area.x2 -= disp_area->x1;
                draw_area.y2 -= disp_area->y1;
                //LV_LOG_USER("sunxifb_g2d_blend start\n");
                if(!sunxifb_g2d_blend(dest_buf, disp_area, &draw_area, (lv_color_t *)src_buf, coords,draw_dsc->opa,chroma_keyed))
                {
                    //LV_LOG_USER("sunxifb_g2d_blend finish\n");
                    return;
                }
            }
        ...
        if (!mask_any && draw_dsc->angle == 0 && draw_dsc->zoom == LV_IMG_ZOOM_NONE
                            && chroma_keyed && draw_dsc->recolor_opa == LV_OPA_TRANSP)
        {
            if(draw_area_h*draw_area_w >= sunxifb_g2d_get_limit(SUNXI_G2D_LIMIT_BLEND))
            {
                /*Make it relative to `disp_area` to simplify draw to `disp_buf`*/
                ...
                draw_area.x2 -= disp_area->x1;
                draw_area.y2 -= disp_area->y1;
                if(!sunxifb_g2d_blend(dest_buf, disp_area, &draw_area, (lv_color_t *)src_buf, &draw_area,draw_dsc->opa,chroma_keyed))
                {
                    return;
                }
            }
