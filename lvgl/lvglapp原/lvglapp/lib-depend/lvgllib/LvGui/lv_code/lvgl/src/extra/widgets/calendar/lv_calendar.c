...
#define LV_CALENDAR_CTRL_MONTH      LV_BTNMATRIX_CTRL_CUSTOM_3
#define LV_CALENDAR_CTRL_WEEKDAY    LV_BTNMATRIX_CTRL_CUSTOM_4
...
static  lv_ft_info_t ftfont;
static lv_opa_t bgOpa=LV_OPA_TRANSP;
static lv_opa_t borderOpa=LV_OPA_TRANSP;
static int btnRadius=100;
static int bgColor= 0x0;
static int bgColorCur= 0xffffff;
static int todayFontColor= 0xf44336;
static int monthFontColor= 0xffffff;
static int unMonthFontColor= 0x03a9f4;
static int weekFontColor= 0xf9c139;
static int curFontColor= 0x03a9f4;
...
void lv_calendar_set_font(int weight,int fontType)
{
    lv_ft_font_destroy(ftfont.font);
    ftfont.weight = weight;
    ftfont.style = fontType;
...
void lv_calendar_set_bgAndBorder(lv_opa_t bgopa, lv_opa_t borderopa,int btnradius,int bgcolor)
{
    bgOpa =bgopa;
    borderOpa =borderopa;
    btnRadius =btnradius;
...
void lv_calendar_set_color(int bgcolorcur,int today,int month,int unMonthr,int week,int cur)
{
    bgColorCur = bgcolorcur;
    todayFontColor = today;
    monthFontColor = month;
    unMonthFontColor = unMonthr;
    weekFontColor = week;
    curFontColor = cur;
}
...
void lv_calendar_set_showed_date(lv_obj_t * obj, uint32_t year, uint32_t month)
{
    ...
    lv_btnmatrix_clear_btn_ctrl_all(calendar->btnm, LV_CALENDAR_CTRL_MONTH|LV_CALENDAR_CTRL_WEEKDAY);
    ...
    for(i = day_first, c = 1; i < act_mo_len + day_first; i++, c++) {
        ...
        weekCur = get_day_of_week(d.year, d.month, c);
        if((weekCur ==0)||(weekCur ==6))
        {
            lv_btnmatrix_set_btn_ctrl(calendar->btnm, i, LV_CALENDAR_CTRL_WEEKDAY);
        }
    }
    ...
    for(i = 0, c = prev_mo_len - day_first + 1; i < day_first; i++, c++) {
        lv_snprintf(calendar->nums[i], sizeof(calendar->nums[0]), "%d", c);
        lv_btnmatrix_set_btn_ctrl(calendar->btnm, i, LV_CALENDAR_CTRL_MONTH);
    }
    ...
    if(lv_btnmatrix_get_selected_btn(calendar->btnm) != LV_BTNMATRIX_BTN_NONE) {
        lv_btnmatrix_set_selected_btn(calendar->btnm, day_first);
    }
...
lv_res_t lv_calendar_get_pressed_date(const lv_obj_t * obj, lv_calendar_date_t * date)
{
    ...
    if(lv_btnmatrix_has_btn_ctrl(calendar->btnm, lv_btnmatrix_get_selected_btn(calendar->btnm),LV_CALENDAR_CTRL_MONTH))
    {
        if(date->day >20)
        {
            date->month --;
...
static void lv_calendar_constructor(const lv_obj_class_t * class_p, lv_obj_t * obj)
{
    ...
    ftfont.name = DEFAULT_FONT_NAME;
    ftfont.weight = 28;
    ftfont.style = FT_FONT_STYLE_NORMAL;
    lv_ft_font_init(&ftfont);
...
static void draw_part_begin_event_cb(lv_event_t * e)
{
    ...
    if(dsc->part == LV_PART_ITEMS) {
        dsc->label_dsc->font = ftfont.font;
        dsc->rect_dsc->bg_opa = bgOpa;
        dsc->rect_dsc->border_opa = borderOpa;
        dsc->rect_dsc->bg_color = lv_color_hex(bgColor);
        /*Day name styles*/
        if(lv_btnmatrix_has_btn_ctrl(obj, dsc->id, LV_CALENDAR_CTRL_MONTH)) {
            dsc->label_dsc->color = lv_color_hex(unMonthFontColor);
        }
        else if(lv_btnmatrix_has_btn_ctrl(obj, dsc->id, LV_CALENDAR_CTRL_WEEKDAY)) {
            dsc->label_dsc->color = lv_color_hex(weekFontColor);
        }
        else
        {
            dsc->label_dsc->color = lv_color_hex(monthFontColor);
        }
        if(lv_btnmatrix_has_btn_ctrl(obj, dsc->id, LV_CALENDAR_CTRL_TODAY)) {
            dsc->label_dsc->color = lv_color_hex(todayFontColor);
        }
        if(lv_btnmatrix_has_btn_ctrl(obj, dsc->id, LV_CALENDAR_CTRL_HIGHLIGHT)) {
            dsc->rect_dsc->bg_opa = LV_OPA_100;
            dsc->rect_dsc->radius = btnRadius;
            dsc->rect_dsc->bg_color = lv_color_hex(bgColorCur);
            dsc->label_dsc->color = lv_color_hex(curFontColor);
        }
    }
}
        ...
        for(i = 0; i < calendar->highlighted_dates_num; i++) {
            if(calendar->highlighted_dates[i].year == calendar->showed_date.year &&
               calendar->highlighted_dates[i].month == calendar->showed_date.month) {
                lv_btnmatrix_set_btn_ctrl(calendar->btnm, calendar->highlighted_dates[i].day - 1 + day_first, LV_CALENDAR_CTRL_HIGHLIGHT);
            }
        }
    }

    if(calendar->showed_date.year == calendar->today.year && calendar->showed_date.month == calendar->today.month) {
        lv_btnmatrix_set_btn_ctrl(calendar->btnm, calendar->today.day - 1 + day_first, LV_CALENDAR_CTRL_TODAY);
    }
}
