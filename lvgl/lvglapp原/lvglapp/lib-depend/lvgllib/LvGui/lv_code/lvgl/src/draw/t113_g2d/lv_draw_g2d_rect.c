...
LV_ATTRIBUTE_FAST_MEM static void shadow_draw_corner_buf(const lv_area_t * coords, uint16_t * sh_buf, lv_coord_t s,
                                                         lv_coord_t r);
LV_ATTRIBUTE_FAST_MEM static void shadow_blur_corner(lv_coord_t size, lv_coord_t g2d, uint16_t * sh_ups_buf);
#endif

static void draw_border_generic(lv_draw_ctx_t * draw_ctx, const lv_area_t * outer_area, const lv_area_t * inner_area,
...
void lv_draw_g2d_rect(lv_draw_ctx_t * draw_ctx, const lv_draw_rect_dsc_t * dsc, const lv_area_t * coords)
{
#if LV_DRAW_COMPLEX
...
void lv_draw_g2d_bg(lv_draw_ctx_t * draw_ctx, const lv_draw_rect_dsc_t * dsc, const lv_area_t * coords)
{
...
static void draw_bg(lv_draw_ctx_t * draw_ctx, const lv_draw_rect_dsc_t * dsc, const lv_area_t * coords)
{
    ...
    lv_draw_g2d_blend_dsc_t blend_dsc = {0};
    ...
    if(!mask_any && dsc->radius == 0 && (grad_dir == LV_GRAD_DIR_NONE)) {
        ...
        lv_draw_g2d_blend(draw_ctx, &blend_dsc);
        return;
    }
    ...
    if(mask_any) {
        for(h = clipped_coords.y1; h <= clipped_coords.y2; h++) {
             ...
             * It saves calculating the final opa in lv_draw_g2d_blend*/
                                              ...
                                              const lv_area_t * coords)
{
    ...
    lv_draw_g2d_blend_dsc_t blend_dsc;
 ...
 * @param sh_buf a buffer to store the result. Its size should be `(g2d + r)^2 * 2`
 * @param g2d shadow width
 * @param r radius
 */
LV_ATTRIBUTE_FAST_MEM static void shadow_draw_corner_buf(const lv_area_t * coords, uint16_t * sh_buf, lv_coord_t g2d,
                                                         lv_coord_t r)
{
    int32_t g2d_ori = g2d;
    int32_t size = g2d_ori  + r;

    lv_area_t sh_area;
    lv_area_copy(&sh_area, coords);
    sh_area.x2 = g2d / 2 + r - 1  - ((g2d & 1) ? 0 : 1);
    sh_area.y1 = g2d / 2 + 1;
    ...
    for(y = 0; y < size; y++) {
        ...
        if(mask_res == LV_DRAW_MASK_RES_TRANSP) {
            ...
            int32_t i;
            sh_ups_tmp_buf[0] = (mask_line[0] << SHADOW_UPSCALE_SHIFT) / g2d;
            for(i = 1; i < size; i++) {
                if(mask_line[i] == mask_line[i - 1]) sh_ups_tmp_buf[i] = sh_ups_tmp_buf[i - 1];
                else  sh_ups_tmp_buf[i] = (mask_line[i] << SHADOW_UPSCALE_SHIFT) / g2d;
            }
        }
    ...
    shadow_blur_corner(size, g2d, sh_buf);
    ...
    if(g2d > 1) {
        uint32_t i;
        uint32_t max_v_div = (LV_OPA_COVER << SHADOW_UPSCALE_SHIFT) / g2d;
        for(i = 0; i < (uint32_t)size * size; i++) {
            if(sh_buf[i] == 0) continue;
            else if(sh_buf[i] == LV_OPA_COVER) sh_buf[i] = max_v_div;
            else  sh_buf[i] = (sh_buf[i] << SHADOW_UPSCALE_SHIFT) / g2d;
        }
...
LV_ATTRIBUTE_FAST_MEM static void shadow_blur_corner(lv_coord_t size, lv_coord_t g2d, uint16_t * sh_ups_buf)
{
    int32_t s_left = g2d >> 1;
    int32_t s_right = (g2d >> 1);
    if((g2d & 1) == 0) s_left--;
    ...
    for(y = 0; y < size; y++) {
        int32_t v = sh_ups_tmp_buf[size - 1] * g2d;
        for(x = size - 1; x >= 0; x--) {
            sh_ups_blur_buf[x] = v;
    ...
    uint32_t max_v_div = max_v / g2d;
    for(i = 0; i < (uint32_t)size * size; i++) {
        if(sh_ups_buf[i] == 0) continue;
        else if(sh_ups_buf[i] == max_v) sh_ups_buf[i] = max_v_div;
        else sh_ups_buf[i] = sh_ups_buf[i] / g2d;
    }

    for(x = 0; x < size; x++) {
        sh_ups_tmp_buf = &sh_ups_buf[x];
        int32_t v = sh_ups_tmp_buf[0] * g2d;
                               ...
                               lv_color_t color, lv_opa_t opa)
{
    lv_area_t a;
    lv_draw_g2d_blend_dsc_t blend_dsc;
    lv_memset_00(&blend_dsc, sizeof(lv_draw_g2d_blend_dsc_t));
