...
static int16_t  window_width = 800;
static int16_t  window_height = 480;
...
void sdl_set_size(int width, int height)
{
    window_width = width;
...
//    printf("x1:%d,y1:%d,x2:%d,y2:%d\n", area->x1, area->y1, area->x2, area->y2);
    ...
    for(y = area->y1; y <= area->y2 && y < disp_drv->ver_res; y++) {
        memcpy(&monitor.tft_fb[y * window_width + area->x1], color_p, w * sizeof(lv_color_t));
        color_p += w;
    }
#endif
...
static void window_create(monitor_t * m)
{
    ...
    m->window = SDL_CreateWindow("TFT Simulator",
                              ...
                              window_width * SDL_ZOOM, window_height * SDL_ZOOM, flag);       /*last param. SDL_WINDOW_BORDERLESS to hide borders*/

    m->renderer = SDL_CreateRenderer(m->window, -1, SDL_RENDERER_SOFTWARE);
    m->texture = SDL_CreateTexture(m->renderer, SDL_PIXELFORMAT_ARGB8888, SDL_TEXTUREACCESS_STATIC, window_width, window_height);
...
#else
    m->tft_fb = (uint32_t *)malloc(sizeof(uint32_t) * window_width * window_height);
    memset(m->tft_fb, 0x44, window_width * window_height * sizeof(uint32_t));
#endif
...
static void window_update(monitor_t * m)
{
    ...
    SDL_UpdateTexture(m->texture, NULL, m->tft_fb, window_width * sizeof(uint32_t));
#else
