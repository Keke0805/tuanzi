...
bool MainAPPStart::send_init_finish_packet(void)
{
    ...
    sendbuf[0]=PACKRT_HEAD;
    sendbuf[1]=PACKRT_CMD_INIT;
    sendbuf[2]=0x04;
    sendbuf[3]=0x55;
    sendbuf[4]=0x66;
    sendbuf[5]=0x55;
    sendbuf[6]=0x66;
    sendbuf[7]=PACKRT_TAIL;
    send_size += 8;
    return BLE_Uart->send(sendbuf,send_size);
}

bool MainAPPStart::send_open_device_packet(void)
{
    BYTE sendbuf[1024];
    int send_size=0;
    memset(sendbuf,0,sizeof(sendbuf));
    ...
    sendbuf[0]=PACKRT_HEAD;
    sendbuf[1]=PACKRT_CMD_DEVICE_CONTROL;
    sendbuf[2]=0x01;
    sendbuf[3]=0x06;
    sendbuf[4]=PACKRT_TAIL;
    send_size += 5;
    return BLE_Uart->send(sendbuf,send_size);
}

bool MainAPPStart::send_open_bluetooth_packet(void)
{
    BYTE sendbuf[1024];
    int send_size=0;
    memset(sendbuf,0,sizeof(sendbuf));
    ...
    sendbuf[0]=PACKRT_HEAD;
    sendbuf[1]=PACKRT_CMD_BLE_CONTROL;
    sendbuf[2]=0x01;
    sendbuf[3]=0x01;
    sendbuf[4]=PACKRT_TAIL;
    send_size += 5;
...
//control
bool MainAPPStart::send_device_control_packet(unsigned char cmd_type)
{
    BYTE sendbuf[1024];
    int send_size=0;
    memset(sendbuf,0,sizeof(sendbuf));
...
bool MainAPPStart::send_bluetooth_control_packet(unsigned char cmd_type)
{
    BYTE sendbuf[1024];
    int send_size=0;
    memset(sendbuf,0,sizeof(sendbuf));
...
bool MainAPPStart::send_rgb_light_control_packet(unsigned char cmd_type)
{
    BYTE sendbuf[1024];
    int send_size=0;
    memset(sendbuf,0,sizeof(sendbuf));
...
bool MainAPPStart::send_microphone_control_packet(unsigned char cmd_type)
{
    BYTE sendbuf[1024];
    int send_size=0;
    memset(sendbuf,0,sizeof(sendbuf));
...
bool MainAPPStart::send_volume_control_packet(unsigned char cmd_type)
{
    BYTE sendbuf[1024];
    int send_size=0;
    memset(sendbuf,0,sizeof(sendbuf));
...
void MainAPPStart::ui_image_decode(void)
{
    ...
    //
    MAIN_MENU_ALARM_1_PNG=(void*)parse_image_from_file((char*)OTHER_ALARM_1_PNG_PATH);
    MAIN_MENU_ALARM_2_PNG=(void*)parse_image_from_file((char*)OTHER_ALARM_2_PNG_PATH);
    //
...
{   void *MAIN_MENU_text_S_PNG=(void*)parse_image_from_file((char*)"./resource/image/background/Butterflies-neon-light.jpg");
    Mainopt = this;
    m_screen = parent;
    ...
    time=new LvcppLabel(m_parent);
    time->set_txt("13:37");
    time->set_pos_x(50);
    time->set_pos_y(720);
    time->txt_font_size(40, 40);
    time->txt_color(lv_color_white(),0);
    time2=new LvcppLabel(m_parent);
    time2->set_txt("FRIDAY");
    time2->set_pos_x(50);
    time2->set_pos_y(770);
    time2->txt_font_size(40, 40);
    time2->txt_color(lv_color_white(),0);
    time3=new LvcppLabel(m_parent);
    time3->set_txt("03/23");
    time3->set_pos_x(50);
    time3->set_pos_y(820);
    time3->txt_font_size(40, 40);
    time3->txt_color(lv_color_white(),0);
    alarm1=new LvcppLabel(m_parent);
    alarm1->set_main_bg_img_src(MAIN_MENU_ALARM_1_PNG);
    alarm1->set_pos_x(50);
    alarm1->set_pos_y(880);
    alarm1->set_txt("");
    alarm1->set_size(20,20);
    alarm1->set_main_bg_color(lv_color_white(),LV_OPA_100);
#if 0
...
void MainAPPStart::lvgl_scroll_anim(void)
{
    ...
    current_function->set_txt("Bluetooth");
        ...
        case 10:
            {
                ...
                else if(target==opt->btn_left->myobj)
                {
                    if(event->code==LV_EVENT_CLICKED)
                    {
                        ...
                        opt->send_microphone_control_packet((unsigned char)(microphone_tmp%3+1));
    ...
    while(1)
    {
        ...
        if(cnt>1)
        {
            ...
            if(cnt>10)
            {
                ...
                if(!bluetooth_init)
                {
                    opt->send_open_bluetooth_packet();
                    bluetooth_init = true;
                }
                cnt = 0;
            }
...
//
void  MainAPPStart::update_time(lv_event_t *event)
{   //获取时间
