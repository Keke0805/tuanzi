    ...
    while(1)
    {
        ...
        if(nRet == VDECODE_RESULT_KEYFRAME_DECODED ||
			...
			if(validNum >= 0 &&
				...
				decoderLog(" vir_addr=%p, nWidth=%d,nHeight=%d\n",image_buf.vir_addr,videoPicture->nWidth,videoPicture->nHeight);
				/*for(int i=0;i<8;i++)
				{
					printf("output[%d] = %02x\n",i,((uint8_t*)(image_buf.vir_addr))[i]);
				}*/
