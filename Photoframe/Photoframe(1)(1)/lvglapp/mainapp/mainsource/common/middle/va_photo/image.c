...
static int image_standardized(image_buffer_t *buf)
{
    ...
    buf->comp = 4;
    buf->width = IMAGE_HOR_RES_MAX;
    buf->height = IMAGE_VER_RES_MAX;
...
static int sunxi_image_scaler_standardized(image_buffer_t *buf)
{
	...
	memset(&dst, 0, sizeof(dst));
	memcpy(&dst,buf,sizeof(image_buffer_t));
	if((buf->width * buf->height)<(IMAGE_HOR_RES_MAX*IMAGE_VER_RES_MAX))
	{
...
static int sunxi_image_scaler(image_buffer_t *buf, unsigned int scaler_width, unsigned int scaler_height)
{
	...
	scaler_width = *ratio_percent * buf->width / 100;
	scaler_height = *ratio_percent * buf->height / 100;
	if(scaler_width* buf->height > scaler_height* buf->width)
	{
		ratio_percent = scaler_width*100 / buf->width;
	}
	else
	{
		ratio_percent = scaler_height*100 / buf->height;
...
static void image_dst_buffer_switch(void)
{
	...
	int ret = image.opr.process->rotate(&image.src,&image.dst_next, IMAGE_ROTATE_90);
	if (ret	!= 0) {
...
int image_show_normal(void)
{
	...
	crop_rect.x = (int)(IMAGE_VER_RES_MAX -image.dst_next.width)/2;;
	crop_rect.y = (int)(IMAGE_HOR_RES_MAX -image.dst_next.height)/2;;
...
static int jpegdecoder_init(char const *filename)
{
	...
	JpegDecoderSetDataSourceBuf(jpegdecoder, srcBuf, srcBufLen, JPEG_DECODE_SCALE_DOWN_4, JpegDecodeOutputDataNV21);
	...
	NV212RGBorBGR(imgFrame->mYuvData, imgFrame->mWidth, imgFrame->mHeight, image.src.vir_addr,0);
	printf("NV212RGBorBGR finish\n");
	image.src.comp = 3;
...
int image_scaler_decode(char *filename, image_buffer_t *buf)
{
	...
	else
	{
		...
		if(is_jpg)
		{
			printf("run to jpegdecoder_init\n");
...
int image_effect_show(char *filename, image_effect_show_mode_t mode)
{
	...
	image_show_before();
	/*
	printf("------------image.dst_next.rotate_angle = %d------------\n",image.dst_next.rotate_angle);
	image.dst_previous.rotate_angle = IMAGE_ROTATE_90;
	ret = image.opr.process->rotate(&image.dst_previous, &image.dst_previous, image.dst_previous.rotate_angle);
	if (ret	!= 0) {
		printf("%s: image rotate failed!\n", __func__);
		return ret;
	}

	image.dst_next.rotate_angle = IMAGE_ROTATE_90;
	ret = image.opr.process->rotate(&image.dst_next, &image.dst_next, image.dst_next.rotate_angle);
	if (ret	!= 0) {
		printf("%s: image rotate failed!\n", __func__);
		return ret;
	}
...
int image_show(char *filename, image_show_mode_t mode)
{
	...
	if(mode == IMAGE_FULL_SCREEN_SCLAER)
	{
		...
		crop_rect.w = image.dst_next.width;
        crop_rect.h = image.dst_next.height;
		print_zxf("crop_rect.w = %d,crop_rect.h = %d\n",crop_rect.w,crop_rect.h);
		image.opr.display->display(&image.dst_next, &crop_rect);//设置显示区域和显示数据
	}
#else
...
void* sunxi_image_standardized(image_buffer_t *buf,unsigned int scaler_w,unsigned int scaler_h)
{
	...
	dst.vir_addr = ionHdle->palloc(buf->width * buf->height * 4);
	dst.phy_addr = ionHdle->cpu_get_phyaddr(dst.vir_addr);
	dst.fd = ionHdle->get_bufferFd(dst.vir_addr);
	dst.comp  = 4;//buf->comp;
	dst.width  = buf->width;//disp_info.disp_rect.w;
	dst.height = buf->height;//disp_info.disp_rect.h;
	dst.length = dst.width * dst.height * dst.comp;
	dst.fmt = IMAGE_FORMAT_ARGB8888;
	...
	//scaler
	sunxi_image_scaler(buf,scaler_w,scaler_h);
